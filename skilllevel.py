...
ACCEPTABLE_LEVELS = ["Novice", "Intermediate", "Advanced", "Expert"]
...
# ====== PROMPTS ======
SYSTEM_SUMMARY_LEVEL = """
You determine the correct Skill Level for a course.
- Analyze the linked course and public sources.
- Choose exactly one from: Novice, Intermediate, Advanced, Expert.
- Return ONLY the JSON requested; no commentary.
"""

USER_LEVEL_TMPL = """
Using this record, research the linked course and determine the single best Skill Level.
...
def coerce_skill_level(val: str) -> str:
    if not val:
        return ""
    ...
    for c in ACCEPTABLE_LEVELS:
        if s == c.lower():
            return c
    return ""


def is_current_level_valid(row: Dict[str, Any]) -> bool:
    raw = row.get("Skill Level") or row.get("skill_level") or row.get("SkillLevel") or ""
    raw = str(raw).strip()
    if not raw:
        return False
    # Consider delimiters; valid only if a single atomic value
    if any(d in raw for d in [";", ",", "|", "/"]):
        return False
    coerced = coerce_skill_level(raw)
    return coerced in ACCEPTABLE_LEVELS


def call_model_for_skill_level(full_row: Dict[str, Any]) -> str:
    prompt = USER_LEVEL_TMPL.format(row_json=json.dumps(full_row, ensure_ascii=False))
    ...
    def do_request(use_web: bool):
        kwargs = {
            "model": MODEL,
            "input": [
                {"role": "system", "content": [{"type": "input_text", "text": SYSTEM_SUMMARY_LEVEL}]},
            ...
            "response_format": {
                ...
                "json_schema": {
                    ...
                    "schema": {
                        ...
                        "properties": {
                            "Skill Level": {"type": "string", "enum": ACCEPTABLE_LEVELS}
                        }
                    },
                    "strict": True,
                },
        ...
        try:
            ...
            kwargs_min = {
                ...
                "input": [
                    {"role": "system", "content": [{"type": "input_text", "text": SYSTEM_SUMMARY_LEVEL}]},
                    {"role": "user",   "content": [{"type": "input_text", "text": prompt + "\n\nReturn ONLY a JSON object with key 'Skill Level'."}]},
                ],
            }
            if use_web:
            ...
            try:
                ...
                cc = get_client().chat.completions.create(
                    ...
                    model=MODEL,
                    messages=[
                        {"role": "system", "content": SYSTEM_SUMMARY_LEVEL},
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            val = str(data.get("Skill Level") or "").strip()
            val = coerce_skill_level(val)
            if not val:
                logging.warning("Skill Level not recognized; defaulting to Novice.")
                val = "Novice"
            return val
            ...
            if attempt < MAX_RETRIES - 1 and getattr(e, "status_code", 500) >= 500:
                ...
                delay = min(delay * 2, 30)
                continue
            logging.error(f"Non-retryable OpenAI error while fetching Skill Level: {e}")
            return "Novice"
        except Exception as e:
            logging.exception(f"Unexpected error while fetching Skill Level: {e}")
            return "Novice"
    return "Novice"
...
def main():
    ...
    with open(INPUT_CSV, newline="", encoding="utf-8-sig") as f:
        ...
        for row in rows:
            ...
            # If current Skill Level is already a single acceptable value, keep as-is
            if is_current_level_valid(row):
                out = canonicalize_full_row(row)
                append_row(out)
                logging.info(f"Skill Level valid ✅: {provider} | {link}")
            else:
                logging.warning(f"Skill Level invalid ❌: {provider} | {link} — fetching via model…")
                level = call_model_for_skill_level(row)
                row["Skill Level"] = level
                out = canonicalize_full_row(row)
                append_row(out)
                logging.info(f"Skill Level updated ✏️: {provider} | {link} → {level}")
            processed += 1
